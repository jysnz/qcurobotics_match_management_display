...
export default function MatchSchedule({ matches, teams, isEliminationPhase = false, isFinalsPhase = false }: MatchScheduleProps) {
  ...
  const shouldScrollSchedule = upcomingMatches.length > 6;

  const renderMatchRow = (match: Match, index: number, isResult: boolean, isGrouped: boolean = false) => (
    <div 
      key={isGrouped ? undefined : `${match.id}-${index}`} 
      className={`grid grid-cols-[80px_1fr_140px_1fr] gap-x-4 items-center py-3 px-8 transition-colors border-b-2 border-black/5 w-full max-w-2xl mx-auto ${
    ...
    >
      ...
      {isResult ? (
        <div className="text-3xl font-black text-black tabular-nums tracking-tighter text-center leading-none bg-black/5 py-2 rounded-lg mx-2 min-w-[120px]">
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-white">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <div className="bg-[#F2F2F2] text-zinc-600 font-black uppercase text-2xl tracking-widest border-b-4 border-zinc-200 py-6 px-10 z-20 shrink-0 text-center">
          RESULTS
        </div>
        <div className="flex-1 overflow-hidden">
          {recentResults.length > 0 ? (
            <Ticker 
              itemCount={shouldScrollResults ? recentResults.length * (logos.length || 1) : recentResults.length} 
              speed={shouldScrollResults ? 0.8 : 0} 
              className="h-full" 
              gap={0}
            >
              {shouldScrollResults && logos.length > 0 ? logos.map((_, logoIndex) => (
                <div key={`results-group-${logoIndex}`}>
                  {recentResults.map((match, index) => renderMatchRow(match, index, true, true))}
      ...
      <div className="flex-1 flex flex-col min-h-0 border-t border-black/10">
        ...
        <div className="flex-1 overflow-hidden">
          {upcomingMatches.length > 0 ? (
            <Ticker 
              itemCount={shouldScrollSchedule ? upcomingMatches.length * (logos.length || 1) : upcomingMatches.length} 
              speed={shouldScrollSchedule ? 0.8 : 0} 
              className="h-full" 
              gap={0}
            >
              {shouldScrollSchedule && logos.length > 0 ? logos.map((_, logoIndex) => (
                <div key={`schedule-group-${logoIndex}`}>
                  {upcomingMatches.map((match, index) => renderMatchRow(match, index, false, true))}
                  <div style={{ height: '16px' }} className="bg-white shrink-0" aria-hidden="true" />
                </div>
              )) : (
                <div className="flex flex-col items-center justify-center h-full py-4">
